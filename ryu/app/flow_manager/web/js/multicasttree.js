...
$(function () {
    ...
    function plotGraph(graph, id) {
        ...
        link.append("text")
            .attr("class", "start")
            .text(function (d) {
                return trim_zeros(d.port.source);
            })

        link.append("text")
            .attr("class", "end")
            .text(function (d) {
                return trim_zeros(d.port.target);
            })
        ...
        function tickActions() {
            ...
            link.selectAll("line")
                .attr("x1", function (d) {
                    return (d.target.x - d.source.x) * size / 2 / norm(d);
                })
                .attr("y1", function (d) {
                    return (d.target.y - d.source.y) * size / 2 / norm(d);
                })
                .attr("x2", function (d) {
                    return (d.target.x - d.source.x) * (1 - size / 2 / norm(d));
                })
                .attr("y2", function (d) {
                    return (d.target.y - d.source.y) * (1 - size / 2 / norm(d));
                })
    ...
    function getTopology() {
        getMulticastTrees(
            function (src_list){
                tabObj.buildTabs("#main", src_list, "Nothing to show!");
            },
    ...
    function getMulticastTrees(f, g) {
        let src_list = [];
        let src_data = [];
        $.get("/multicasttrees")
            .done(function (trees) {
                ...
                f(src_list)
                $.when.apply(this, src_data).then(function () {
                    g(src_list, src_data);
                });
            })
            .fail(function () {
